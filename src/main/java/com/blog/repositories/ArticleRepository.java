...
public interface ArticleRepository extends JpaRepository< Article, String> {
    ...
    @Query(value = "SELECT A.id, A.title, A.description, A.tags, A.image_url as imageUrl , " +
            " A.num_views as numViews, A.num_shares as numShares, A.url_friendly as urlFriendly, A.type, A.create_at as createAt, A.num_like as num_like  FROM article A where MATCH(title, description) AGAINST (:textSearch)", nativeQuery = true)
    ...
    @Query("SELECT new com.blog.dto.ArticleShortDto(A.id, A.title, A.description, A.tags, A.imageUrl, " +
            "  A.numViews, A.numShares, A.urlFriendly, A.type, A.createAt, A.numLike)  from Article A where A.type = :type order by A.createAt desc")
    List<ArticleShortDto> getByType(ArticleEnum type, Pageable pageable);

    @Query("SELECT new com.blog.dto.ArticleShortDto(A.id, A.title, A.description, A.tags, A.imageUrl," +
            " A.numViews, A.numShares, A.urlFriendly, A.type, A.createAt, A.numLike) from Article A order by A.numViews desc")
    List< ArticleShortDto > getOrderByViews(Pageable pageable);

    @Query("SELECT new com.blog.dto.ArticleShortDto(A.id, A.title, A.description, A.tags, A.imageUrl," +
            " A.numViews, A.numShares, A.urlFriendly, A.type, A.createAt, A.numLike) from Article A order by A.createAt desc")
    List<ArticleShortDto> getOrderByTime(Pageable pageable);

    @Query("SELECT new com.blog.dto.ArticleShortDto(A.id, A.title, A.description, A.tags, A.imageUrl," +
            " A.numViews, A.numShares, A.urlFriendly, A.type, A.createAt, A.numLike) from Article A " +
            " JOIN Like l on l.articleId =  A.id AND l.createBy = :userId")
    List<ArticleShortDto> getByLike(Pageable pageable, String userId);

    @Query("SELECT new com.blog.dto.ArticleShortDto(A.id, A.title, A.description, A.tags, A.imageUrl," +
            " A.numViews, A.numShares, A.urlFriendly, A.type, A.createAt, A.numLike) from Article A WHERE A.tags like %:tag%")
